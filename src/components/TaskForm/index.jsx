...
const TaskForm = () => {
  ...
  const handleSubmit = event => {
    ...
    // TODO: submit { type: formValues.type, priority: Number(formValues.priority), payload: parsedPayload.value } to POST /formValues.
  };
      ...
      <form noValidate onSubmit={handleSubmit} className="mt-5 space-y-5">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            <label htmlFor={typeId} className="block text-sm font-medium text-slate-200">
              Type
            </label>
            <select
              id={typeId}
          ...
          <div className="space-y-1.5">
            <label htmlFor={priorityId} className="block text-sm font-medium text-slate-200">
              Priority
            </label>
            <select
              id={priorityId}
        ...
        <div className="space-y-1.5">
          <label htmlFor={payloadId} className="block text-sm font-medium text-slate-200">
            Payload <span className="font-normal text-slate-400">(optional JSON object)</span>
          </label>
          <textarea
            id={payloadId}
            name="payload"
